...
export default function OversightScreen() {
  ...
  return (
    <SafeAreaView className="flex-1 bg-gray-100 dark:bg-gray-900">
      ...
      {/* Tabs */}
      <View className="flex-row mx-5 mb-4 rounded-xl overflow-hidden" style={{ backgroundColor: isDark ? '#1f2937' : '#f3f4f6' }}>
        {(['month', 'year'] as Tab[]).map((tab) => (
          <TouchableOpacity
            key={tab}
            onPress={() => setActiveTab(tab)}
            className="flex-1 py-3 items-center rounded-xl"
            style={activeTab === tab ? { backgroundColor: activeBg } : undefined}
          >
            <Text
        ...
        {activeTab === 'month' ? (
          <View>
            ...
            <View className="flex-row items-center justify-between mb-4">
              ...
              <Text weight="bold" className="text-base capitalize" style={{ color: textColor }}>
                {monthLabel}
              </Text>
              <TouchableOpacity
